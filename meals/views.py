...
class MealOptionListCreateView(APIView):
    ...
    def post(self, request):
        serializer = MealOptionSerializer(data=request.data, context={"request": request})
...
class DailyMenuListCreateView(APIView):
    ...
    def post(self, request):
        serializer = DailyMenuSerializer(data=request.data, context={"request": request})
        if serializer.is_valid():
...
class DailyMenuItemCreateView(APIView):
    ...
    def post(self, request):
        serializer = DailyMenuItemSerializer(data=request.data, context={"request": request})
        if serializer.is_valid():
